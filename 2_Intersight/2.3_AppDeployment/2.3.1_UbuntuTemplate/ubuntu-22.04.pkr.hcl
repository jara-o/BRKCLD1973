...
variable "ssh_host" {
  type        = string
  description = "The VM IP to SSH into."
  default     = ""
  sensitive   = true
}
...
source "vsphere-iso" "linux-ubuntu2204-server" {
  ...
  iso_url = var.iso_url
  // iso_path = ["[${ var.vcenter_datastore }] /${ var.iso_path }/${ var.iso_file }"]
...
build {
  sources = ["source.vsphere-iso.linux-ubuntu2204-server"]
  provisioner "shell" {
